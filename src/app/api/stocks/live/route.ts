...
export async function GET(request: Request) {
  try {
    ...
    const basePrice = mock?.price || (50 + Math.random() * 400);
    const priceVariation = (Math.random() - 0.5) * 2;
    const currentPrice = Math.round((basePrice + priceVariation) * 100) / 100;

    const stockData = {
      symbol,
      name: mock?.name || symbol,
      price: currentPrice,
      previousClose: Math.round((currentPrice - (mock?.change || 0)) * 100) / 100,
      change: Math.round((mock?.change || (Math.random() - 0.5) * 10) * 100) / 100,
      changePercent: Math.round((mock?.changePercent || (Math.random() - 0.5) * 4) * 100) / 100,
      ...
      marketCap: mock?.marketCap || Math.floor(Math.random() * 2000000000000 + 500000000000),
      peRatio: mock?.peRatio || Math.round((15 + Math.random() * 40) * 100) / 100,
      week52High: mock?.week52High || Math.round((basePrice + Math.random() * 50) * 100) / 100,
      week52Low: mock?.week52Low || Math.round((basePrice - Math.random() * 50) * 100) / 100,
      timestamp: now,
    };
